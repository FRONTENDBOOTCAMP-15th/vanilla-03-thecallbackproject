...
### 🪻 소혜림
이번 프로젝트를 통해 팀 작업은 개인 의욕만으로 밀어붙인다고 굴러가는 것이 아니라, <br>각자의 상황과 속도를 이해하며 함께 맞추는 과정이 중요하다는 것을 배웠다. <br>초기 설계 단계에서 모듈화를 충분히 하지 않아 뒤늦게 더 큰 수정 비용을 치른 경험은 <br>“구조는 처음부터 잡아야 한다”는 교훈을 남겼다. <br>한편 배포를 제때 진행하지 못해 일정이 촉박해졌고, 그로 인해 추가 기능을 시도하지 못한 점은 아쉬움으로 남지만, <br>API를 받아 화면에 렌더링하기까지의 전체 흐름을 직접 구현하며 데이터 기반 UI의 구조를 온전히 이해하게 된 점은 큰 수확이었다.
