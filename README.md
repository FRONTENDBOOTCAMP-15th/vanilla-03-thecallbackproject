...
<table>
  ...
  <tr>
    <td><b>소혜림</b></td>
    <td>
      - 이미지 첨부 시 파일 서버 타임아웃으로 글쓰기 실패<br>
      - 렌더링 시 ARIA 접근성 속성 누락<br>
      - 이미지 첨부 취소 기능 누락<br>
      - 태그 입력창 자동완성 비활성화 누락<br>
      - 최근 본 글 저장 로직 누락<br>
      - 모바일 가상 키보드 노출 시 툴바 가려짐
    </td>
    <td>
      - 2MB 업로드 제한 적용하여 업로드 시간 단축<br>
      - 요소 생성 시 필요한 ARIA 속성을 함께 추가하도록 렌더링 로직 수정<br>
      - 파일 input 초기화 + 미리보기 삭제 기능 구현<br>
      - autocomplete="off" 추가로 자동완성 비활성화<br>
      - 상세페이지 진입 시 글 데이터를 로컬스토리지에 저장하도록 구현<br>
      - visualViewport의 height 변화를 감지해 툴바 위치를 동적으로 조정
    </td>
  </tr>
